...
const NotificacionAdmin = ({
  ...
  const filteredNotificaciones = sortedNotificaciones.filter(item => {
    const notificationDate = item.fecha.toDate()
    const sevenDaysAgo = subDays(new Date(), 1)
    return notificationDate >= sevenDaysAgo
  })

  return (
      <div className='fixed top-0 left-0 w-full h-full flex justify-center items-center px-3 bg-black bg-opacity-80 z-[100]'>
          {showingNotifications
            ...
            {filteredNotificaciones.length > 0 && (
              <div className='mb-8 mx-auto flex flex-col'>
                <div className='flex flex-col w-full mx-auto gap-y-2'>
                  {filteredNotificaciones.map((item, index) => (
                    <div key={index} className='text-[#0D0D0D] font-bold mx-auto'>
                      <p>
                        - {item?.usuario?.nombre} {item?.usuario?.apellido} {item.tipo === 'cancelada' ? 'no asistirá' : 'asistirá'} a la clase del {format(item.fecha.toDate(), 'dd/MM/yyyy')}
                      </p>
                    </div>
                  ))}
                </div>
            ...
            <div className='flex flex-col mx-auto w-full gap-y-2'>
              {localNotas.map((item, index) => (
                <div key={index} className='flex mx-auto gap-x-6'>
                  ...
                  {editingNotes && (
                    <p className='text-orange-600 md:cursor-pointer my-auto text-base font-bold md:hover:text-orange-300' onClick={() => deleteNoteLocally(index)}>X</p>
                  )}
                </div>
              ))}
            </div>
            {editingNotes && (
              <div className='flex flex-col mx-auto mt-4'>
                ...
                <div className='text-orange-600 text-base font-bold md:hover:text-orange-300'>
            ...
            <div className='flex justify-center gap-x-12 sm:gap-x-16 mt-8'>
              {editingNotes
                ? (
                <>
                  <button className='text-orange-600 font-bold md:hover:text-orange-300' onClick={updateNotes}>
                    Guardar
                  </button>
                  <button className='text-orange-600 md:hover:text-orange-300 font-bold' onClick={toggleEditing}>
                    Cancelar
                  </button>
                </>
                  )
                : (
                <>
                  <button className='text-orange-600 font-bold md:hover:text-orange-300' onClick={toggleEditing}>
                    Editar notas
                  </button>
                  <button className='text-orange-600 font-bold md:hover:text-orange-300' onClick={toggleView}>
                    Volver
