...
const Buscador = ({ cambios, setCambios }) => {
  ...
  const buscarAlumnos =
    searchTerm.trim() !== ''
      ? alumnos.filter(
        (alumno) =>
          normalizeString(alumno.usuario.full_name.nombre).includes(
            searchTermNormalized
          ) ||
            normalizeString(alumno.usuario.full_name.apellido).includes(
              searchTermNormalized
            )
      )
      : []
  ...
  const buscarProfesores =
    searchTerm.trim() !== ''
      ? profesores.filter(
        (profesor) =>
          normalizeString(profesor.usuario.full_name.nombre).includes(
            searchTermNormalized
          ) ||
            normalizeString(profesor.usuario.full_name.apellido).includes(
              searchTermNormalized
            )
      )
      : []
  ...
  return (
    <div
      id="Buscar"
      className={`flex flex-col bg-[#212121] pt-24 sm:py-24 ${
