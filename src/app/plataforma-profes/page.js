              ...
              <div className='grid grid-cols-1 grid-rows-1 h-screen md:h-auto'>
                ...
                <div className="col-start-1 col-end-1 row-start-1 row-end-1 w-full m-auto z-40 flex flex-col">
                  ...
                  {availableDays.map((day, index) => (
                    <button key={index} onClick={() => filterAlumnosByDay(day)} className="bg-[#FFFFFF] font-botones font-bold text-[#0D0D0D] sm:text-lg rounded-3xl mx-auto h-10 sm:h-8 md:h-8 lg:h-10 w-4/6 sm:w-3/6 md:w-2/6 mb-6 md:mb-4 xl:mb-6">
                      {day}
                    </button>
                  ))}
                </div>
