...
const Buscador = ({ cambios, newCambio }) => {
  ...
  return (
    <div id='Buscar' className='flex flex-col bg-[#212121] pt-12 pb-8 sm:py-12'>
      <h2 className='text-center text-2xl sm:text-3xl text-[#FFFFFF]'>Buscar usuarios</h2>
      <div className="flex text-[#0D0D0D] bg-[#FFFFFF] w-5/6 mb-6 sm:mb-4 sm:w-4/6 md:w-3/6 mx-auto text-center py-2 mt-12 rounded-3xl px-4">
      ...
      {searchTerm.trim() !== '' && (
        <div className='flex flex-col mx-auto w-full'>
          {buscarAlumnos.map((alumno) => (
            <button key={alumno.id} onClick={() => handleAlumnoClick(alumno)} className='mx-auto my-2 bg-[#E9500E] font-botones text-[#FFFFFF] rounded-3xl h-12 sm:h-10 w-4/6 sm:w-2/6 md:hover:bg-[#DB9B6D]'>{alumno.Nombre} {alumno.Apellido} - Alumno</button>
          ))}
        </div>
      )}
      {selectedAlumno && (
      ...
      {searchTerm.trim() !== '' && (
        <div className='flex flex-col mx-auto w-full'>
          {buscarProfesores.map((profesor) => (
            <button key={profesor.id} onClick={() => handleProfesorClick(profesor)} className='mx-auto my-2 bg-[#663481] text-[#FFFFFF] font-botones rounded-3xl h-12 sm:h-10 w-4/6 sm:w-2/6 md:hover:bg-[#9B70BE]'>{profesor.Nombre} {profesor.Apellido} - Profesor</button>
          ))}
        </div>
      )}
      {selectedProfesor && (
