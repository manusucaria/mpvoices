    ...
    <div className='w-full'>
      <div className='flex flex-col w-full mx-auto mb-8'>
        ...
        <div className='flex justify-between w-full mx-auto mb-4'>
          <div className={`w-2/6 border-b-1 ${tab === 'datos' ? 'border-b-[#DB9B6D]' : 'border-b-[#FFFFFF]'}`}>
            <button
              className={`font-botones font-bold py-2 mr-4 hover:text-[#DB9B6D] rounded-t-lg ${tab === 'datos' ? 'text-[#DB9B6D]' : 'text-[#FFFFFF]'}`}
              onClick={() => handleTabChange('datos')}
            >
              Datos
            </button>
          </div>
          <div className={`w-2/6 border-b-1 ${tab === 'clases' ? 'border-b-[#DB9B6D]' : 'border-b-[#FFFFFF]'}`}>
            <button
              className={`font-botones font-bold py-2 hover:text-[#DB9B6D] rounded-t-lg ${tab === 'clases' ? 'text-[#DB9B6D]' : 'text-[#FFFFFF]'}`}
              onClick={() => handleTabChange('clases')}
            >
              Clases
            </button>
          </div>
          <div className={`w-2/6 border-b-1 ${tab === 'pagos' ? 'border-b-[#DB9B6D]' : 'border-b-[#FFFFFF]'}`}>
            <button
              className={`font-botones font-bold py-2 hover:text-[#DB9B6D] rounded-t-lg ${tab === 'pagos' ? 'text-[#DB9B6D]' : 'text-[#FFFFFF]'}`}
