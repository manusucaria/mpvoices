    ...
    <div className='w-full'>
      <div className='flex flex-col w-full mx-auto'>
        <div className='w-full flex justify-center pb-6 sm:pb-8 mx-auto gap-x-4 sm:gap-x-6'>
          ...
          <p className='text-[#F57B48] my-auto md:mt-1 text-xl sm:text-2xl mr-auto'>{alumno.Nombre} {alumno.Apellido}</p>
        </div>
        <div className='flex justify-between w-full mx-auto bg-[#0D0D0D] px-4 sm:px-8 py-8'>
          <div className={`w-2/6 border-b-1 ${tab === 'datos' ? 'border-b-[#F57B48]' : 'border-b-[#FFFFFF]'}`}>
            <button
              className={`font-botones font-bold py-2 mr-4 ${tab === 'datos' ? '' : 'hover:text-[#F57B48]'} rounded-t-lg ${tab === 'datos' ? 'text-[#F57B48]' : 'text-[#FFFFFF]'}`}
              onClick={() => handleTabChange('datos')}
            >
              Datos
            </button>
          </div>
          <div className={`w-2/6 border-b-1 ${tab === 'clases' ? 'border-b-[#F57B48]' : 'border-b-[#FFFFFF]'}`}>
            <button
              className={`font-botones font-bold py-2 ${tab === 'clases' ? '' : 'hover:text-[#F57B48]'} rounded-t-lg ${tab === 'clases' ? 'text-[#F57B48]' : 'text-[#FFFFFF]'}`}
              onClick={() => handleTabChange('clases')}
            >
              Clases
            </button>
          </div>
          <div className={`w-2/6 border-b-1 ${tab === 'pagos' ? 'border-b-[#F57B48]' : 'border-b-[#FFFFFF]'}`}>
            <button
              className={`font-botones font-bold py-2 ${tab === 'pagos' ? '' : 'hover:text-[#F57B48]'} rounded-t-lg ${tab === 'pagos' ? 'text-[#F57B48]' : 'text-[#FFFFFF]'}`}
